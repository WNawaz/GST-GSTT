...
str p_suffix):
    ...
    cdef:
        ...
        cnp.float64_t sigma = 0, sigmat = 0, G_gcc = 0, e_gcc = 0, e_gcct = 0, beta = 0
        str INP_DEG = ''
        str INP_WDEG = ''
        str INP_CUDEG = ''
        str INP_ADJ = ''
    ...
    if 'No' in p_suffix:
        ...
        beta = p_scaling * p_scaling
    elif p_suffix == 'E3w':
        sigma = 0.5 * (p_scaling * p_scaling * p_scaling + p_scaling)
        beta = 0.5 * (p_scaling * p_scaling + p_scaling)
    ...
    if 'GDCC' in p_suffix:
        ...
        for i in range(p_n):
            if G_deg_v[i] > 1:
                ewe_v[i] = <float>(0.5 * beta * G_deg_v[i] * (G_deg_v[i] - 1) - sigma * G3c_v[i])
                Gwe_v[i] = <float>(0.5 * G_deg_v[i] * (G_deg_v[i] - 1) - G3c_v[i])
        print('wedges done!!!!!!!!!!!!')
    elif p_suffix == 'E3w':
        # wedges
